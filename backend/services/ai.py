...
输出格式要求（参考 36氪 / 虎嗅 / 财新早报风格，极简排版）：

—— 今日核心 ——
1-2 句话，概括今天最重要的趋势或事件。

—— 重点速览 ——
每条不超过 2 行，写 4-6 条，每条只说最关键的：事件是什么 + 为什么重要。

—— 一句话洞察 ——
1 句独立的深度观察，不是重复新闻，是你的判断。

排版规则：
- 不使用 emoji、不使用 markdown 星号加粗、不使用项目符号列表
- 章节标题用"—— 标题 ——"这种形式
- 正文简洁有力，一句一段，用空行分隔
- 语气专业克制，像早报而非社交媒体
...
{news_text}"""
    ...
    return deepseek_call(
        prompt,
        system="你是一位资深 AI 行业分析师，擅长从海量信息中提炼最核心的洞察。语言简洁克制，像专业早报编辑。不用emoji，不用markdown加粗，不用项目符号。一句一段，用空行分隔。", 
        temperature=0.5,
        max_tokens=1500,
    )
